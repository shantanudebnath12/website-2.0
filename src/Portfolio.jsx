...
function useScrollSpy(rootRef, ids) {
  ...
  useEffect(() => {
    const elements = ids.map((id) => document.querySelector(`[data-section="${id}"]`)).filter(Boolean);
    const io = new IntersectionObserver(
      ...
      { root: null, threshold: [0.3, 0.55, 0.8] }
    );
...
function Reveal({ children, delay = 0, from = "up", rootRef }) {
  ...
  useEffect(() => {
    ...
    if (rb.top < window.innerHeight && rb.bottom > 0) { setShown(true); return; }
    const io = new IntersectionObserver(
...
function Portfolio({ variant = "classic", initialPalette = "green", initialMode = "dark" }) {
  ...
  return (
    <div ref={scopeRef} style={{
      ...cssVars, position: "relative", width: "100%", minHeight: "100dvh",
      fontFamily: fontBody, color: pal.zoneAInk,
    }}>
      {/* Nav */}
      <nav style={{
      ...
      {isMobile && menuOpen && (
        <div style={{
          position: "fixed", top: 58, left: 12, right: 12, zIndex: 55,
      ...
      {/* Particles overlay */}
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 1, opacity: 0.5 }}>
        <DotsBG />
      </div>
